...
public:
	...
	void ReceiveError(const Result &result)
	{
		::printf("receive error # %s\n",result.ErrorMessage().c_str());
	}

	void ResponseError(const Result &result)
	{
...
public:
	void Run()
	{
		...
		for (int i = 0; i < notify_count; i++) {
			...
			Result res = server->Notify(update);
			if (!res) {
...
public:
	void Run()
	{
		...
		while (start_client_request) {
			...
			req.Append(b);
			req.Append(c);
			Result result = client->SendReceive(req,res);
			if (!result) {
				printf("error # %s\n", result.ErrorMessage().c_str());
			} else {
				int d;
